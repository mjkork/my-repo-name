...
- 26 columns, covering every Session field: basics (`date`, `name`, `bow_name`, `location`, `distance_m`), shooting details, environmental conditions, personal state, session experience, and reflection fields. Exact order is defined by `EXPORT_COLUMNS` in `preferences/views.py`. There is no `session_id` column — the DB primary key has gaps (deleted sessions) and can be out of chronological order (backfilled sessions), and doesn't serve any analysis need; `date` alone is a sufficient identifier since every session has a unique date.
